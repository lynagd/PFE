        ...
        <table className="min-w-full bg-[#faf8f3] rounded-xl shadow font-sans">
          <thead>
            <tr>
              ...
              <th className="py-3 px-4 text-khder font-bold text-base border-b-2 border-lsecondary text-left"></th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((patient, idx) => (
              <tr key={patient.id} className={idx % 2 === 1 ? "bg-smth" : ""}>
                ...
                {/* Only show the add patient button on the last row */}
                {idx === filtered.length - 1 && (
                  <td className="py-4 px-4" rowSpan={filtered.length}>
                    <button
                      className="bg-yellow-500 text-[#222] font-semibold rounded-lg px-4 py-2 shadow hover:bg-yellow-600 transition text-sm"
            ...
            {/* If no patients, show a row with the add button */}
            {filtered.length === 0 && (
              <tr>
